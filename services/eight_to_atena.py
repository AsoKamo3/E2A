...
# Eight CSV/TSV → 宛名職人CSV 変換本体 v2.53
# - 既存ロジックは維持
# - PARTIAL_ACRONYM_MAX_LEN（既定3）を導入し、略称の1文字読み上げを短い塊に限定
# - “部分一致”に左→右の貪欲最長一致スキャナ＋未マッチ区間の推測埋め
# - 起動とランタイムの軽量化：
#   * 会社辞書ローダを LRU キャッシュ（maxsize=1）
#   * 法人格（漢字系）を可変セパレータ許容の事前コンパイル済み正規表現で一括除去
#   * _strip_company_type に LRU（maxsize=8192）
...
# --- 可変セパレータ許容の漢字系法人格を事前コンパイル ---
# 中黒・全半角スペース・スラッシュ・各種ダッシュ等を挟み込み許容
_VAR_SEP_CLASS = r"[ 　・／/,\.\-‐─―－\(\)\[\]【】]*"

# よく出る“漢字系”法人格（順序は長い語を先に）
_KANJI_TYPE_PATTERNS: List[str] = [
    "一般社団法人", "一般財団法人",
    "特定非営利活動法人", "地方独立行政法人", "独立行政法人", "国立研究開発法人",
    "医療法人社団", "医療法人財団", "医療法人",
    "公益社団法人", "公益財団法人",
    "社会保険労務士法人", "社会福祉法人",
    "公立大学法人", "国立大学法人", "学校法人",
...
def _build_kanji_type_re() -> "re.Pattern[str]":
    # 各文字の間に可変セパレータを許容（例：「一 般・社／団　法人」もマッチ）
    def intersperse(var: str, s: str) -> str:
        parts = [re.escape(ch) for ch in s]
        return var.join(parts)
    pats = [intersperse(_VAR_SEP_CLASS, s) for s in _KANJI_TYPE_PATTERNS]
    # 長い語優先の OR
    return re.compile("|".join(pats))

_KANJI_TYPE_RE = _build_kanji_type_re()
...
def _strip_company_type(name: str) -> str:
    ...
    if not base:
        return ""
    # 1) 可変セパレータ許容の漢字系法人格を一括除去
    base = _KANJI_TYPE_RE.sub("", base)
    # 2) 英語系・短縮表記などの単純置換
